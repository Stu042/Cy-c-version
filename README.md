...
Basic types have the type (starting with a lowercase letter) followed by a number to indicate bit size. Bit size must be a power of 2:
...
Each function that could be run on a thread - other than thread #0 - has two entry points.
